...
func TestRun_FlagValidation(t *testing.T) {
	cases := []struct {
		...
		{
			flags: []string{
				"-consul-k8s-image", "foo", "-consul-image", "foo", "-envoy-image", "envoy:1.16.0",
				"-log-level", "invalid",
			},
		...
		{
			flags: []string{
				"-consul-k8s-image", "hashicorpdev/consul-k8s:latest", "-consul-image", "foo", "-envoy-image", "envoy:1.16.0",
