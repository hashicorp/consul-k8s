...
func CheckForPriorInstallations(t *testing.T, client kubernetes.Interface, options *helm.Options, chartName, label string) {
	t.Helper()
	...
	retry.RunWith(&retry.Counter{Wait: 1 * time.Second, Count: 60}, t, func(r *retry.R) {
		pods, err := client.CoreV1().Pods(options.KubectlOptions.Namespace).List(context.Background(), metav1.ListOptions{LabelSelector: label})
		require.NoError(r, err)
		if len(pods.Items) > 0 {
			var podNames []string
