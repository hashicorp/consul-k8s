	...
	for _, c := range cases {
		t.Run(c.name, func(t *testing.T) {
			...
			if cfg.EnableWindows {
				...
				k8s.DeployKustomize(t, primaryStaticServerOpts, cfg.NoCleanupOnFailure, cfg.DebugDirectory, "../fixtures/bases/static-server-windows")
				// create service in secondary partition.
				k8s.DeployKustomize(t, secondaryStaticServerOpts, cfg.NoCleanupOnFailure, cfg.DebugDirectory, "../fixtures/bases/static-server-windows")
			} else {
				// create service in default partition.
