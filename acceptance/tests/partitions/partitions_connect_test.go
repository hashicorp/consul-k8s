...
type partitionsConnectCase struct {
	destinationNamespace string
	mirrorK8S            bool
	ACLsEnabled          bool
}

// runPartitionsConnect tests that Connect works in default and ACLs-enabled installations
// for both cross-partition and in-partition networking. The nested in-partition and
// cross-partition sub-tests share cluster state and must remain sequential inside this helper.
func runPartitionsConnect(t *testing.T, c partitionsConnectCase) {
	t.Helper()
	...
	// The in-partition and cross-partition sub-tests share the cluster state created above
	// and must remain sequential here.
	t.Run("in-partition", func(t *testing.T) {
		...
		// Consul namespace with the same name as their source Kubernetes namespace.
		...
		if c.ACLsEnabled {
			...
			// Set the destination namespace to be the same unless mirrorK8S is true.
			if !c.mirrorK8S {
		...
		// there will be no healthy proxy host to connect to.
...
func TestPartitions_Connect_DefaultNamespace(t *testing.T) {
	runPartitionsConnect(t, partitionsConnectCase{destinationNamespace: defaultNamespace, mirrorK8S: false, ACLsEnabled: false})
}

func TestPartitions_Connect_DefaultNamespaceACLs(t *testing.T) {
	runPartitionsConnect(t, partitionsConnectCase{destinationNamespace: defaultNamespace, mirrorK8S: false, ACLsEnabled: true})
}

func TestPartitions_Connect_SingleNamespace(t *testing.T) {
	runPartitionsConnect(t, partitionsConnectCase{destinationNamespace: staticServerNamespace, mirrorK8S: false, ACLsEnabled: false})
}

func TestPartitions_Connect_SingleNamespaceACLs(t *testing.T) {
	runPartitionsConnect(t, partitionsConnectCase{destinationNamespace: staticServerNamespace, mirrorK8S: false, ACLsEnabled: true})
}

func TestPartitions_Connect_MirroredNamespaces(t *testing.T) {
	runPartitionsConnect(t, partitionsConnectCase{destinationNamespace: staticServerNamespace, mirrorK8S: true, ACLsEnabled: false})
}

func TestPartitions_Connect_MirroredNamespacesACLs(t *testing.T) {
	runPartitionsConnect(t, partitionsConnectCase{destinationNamespace: staticServerNamespace, mirrorK8S: true, ACLsEnabled: true})
}
