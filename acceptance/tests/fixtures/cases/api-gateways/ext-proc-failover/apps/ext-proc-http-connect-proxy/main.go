...
func decideHandler(routerURL string, httpc *http.Client) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		startedAt := time.Now()
		log.Printf("ext-proc-http-connect-proxy id=%d incoming method=%s remote=%s path=%s host=%s http_headers=[%s]",
		...
		if err != nil {
			log.Printf("ext-proc-http-connect-proxy id=%d failed to read body: %v", id, err)
			http.Error(w, "bad request", http.StatusBadRequest)
			return
		}
		log.Printf("ext-proc-http-connect-proxy id=%d ENVOY-PROCESSING-REQUEST raw_body=%q", id, string(body))
		var procReq processingRequest
		if err := json.Unmarshal(body, &procReq); err != nil {
			log.Printf("ext-proc-http-connect-proxy id=%d failed to parse ProcessingRequest: %v — body: %.200s", id, err, body)
			http.Error(w, "bad request", http.StatusBadRequest)
			return
		}
		log.Printf("ext-proc-http-connect-proxy id=%d ENVOY-REQUEST-HEADERS [%s]", id, formatEnvoyHeaders(procReq.RequestHeaders))
		...
		// Call http-decider-connect-proxy with path and route_key as query params.
		u, _ := url.Parse(routerURL)
		q := u.Query()
		q.Set("path", path)
		q.Set("route_key", routeKey)
		u.RawQuery = q.Encode()
		log.Printf("ext-proc-http-connect-proxy id=%d path=%q route_key=%q -> %s", id, path, routeKey, u.String())

		req, err := http.NewRequestWithContext(context.Background(), http.MethodGet, u.String(), nil)
		if err != nil {
			log.Printf("ext-proc-http-connect-proxy id=%d failed to build request: %v", id, err)
			sendProcessingResponse(w, id, "service-f", startedAt)
			return
		}

		resp, err := httpc.Do(req)
		if err != nil {
			log.Printf("ext-proc-http-connect-proxy id=%d decider call failed after %s: %v", id, time.Since(startedAt), err)
			sendProcessingResponse(w, id, "service-f", startedAt)
			return
		}
		defer resp.Body.Close()
		if resp.StatusCode != http.StatusOK {
			log.Printf("ext-proc-http-connect-proxy id=%d decider returned status=%d after %s", id, resp.StatusCode, time.Since(startedAt))
		}

		respBody, err := io.ReadAll(io.LimitReader(resp.Body, 128))
		if err != nil {
			log.Printf("ext-proc-http-connect-proxy id=%d read body failed: %v", id, err)
			sendProcessingResponse(w, id, "service-f", startedAt)
			return
		}

		target := strings.TrimSpace(string(respBody))
		log.Printf("ext-proc-http-connect-proxy id=%d decider -> %q duration=%s", id, target, time.Since(startedAt))
		switch target {
		case "service-f", "service-g":
		default:
			target = "service-f"
		}
...
// to set x-route-target and pass to the connect-proxy inbound so the service-e1
// application can read it and route the request to service-f or service-g.
func sendProcessingResponse(w http.ResponseWriter, id uint64, target string, startedAt time.Time) {
			...
			Response: commonResponse{
				...
				HeaderMutation: &headerMutation{
					SetHeaders: []headerValueOption{
						{Header: envoyHeader{Key: "x-route-target", RawValue: base64.StdEncoding.EncodeToString([]byte(target))}},
						{Header: envoyHeader{Key: "x-ext-proc-cp", RawValue: base64.StdEncoding.EncodeToString([]byte("applied"))}},
					},
				},
			},
		},
	...
	if _, err := w.Write(respJSON); err != nil {
		log.Printf("ext-proc-http-connect-proxy id=%d failed to write response: %v", id, err)
		return
	}
	log.Printf("ext-proc-http-connect-proxy id=%d RESPONSE target=%q status=200 body=%q total_duration=%s", id, target, string(respJSON), time.Since(startedAt))
}
...
func main() {
	...
	if routerURL == "" {
		routerURL = "http://http-decider-connect-proxy.virtual.consul:9500/decide"
	}
	...
	log.Printf("ext-proc-http-connect-proxy listening on %s, router_url=%s, timeout=%s", addr, routerURL, routerTimeout)

	mux := http.NewServeMux()
	mux.HandleFunc("/decide", decideHandler(routerURL, httpc))
	...
	if err := http.ListenAndServe(addr, mux); err != nil {
		log.Fatalf("ext-proc-http-connect-proxy failed: %v", err)
	}
}
