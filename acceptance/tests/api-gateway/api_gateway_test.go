...
func TestAPIGateway_Basic(t *testing.T) {
	...
	for _, c := range cases {
		...
		t.Run(name, func(t *testing.T) {
			...
			k8s.RunKubectl(t, ctx.KubectlOptions(t), "wait", "--for=condition=available", "--timeout=10m", fmt.Sprintf("deploy/%s", "static-server"))
			...
			k8s.RunKubectl(t, ctx.KubectlOptions(t), "wait", "--for=condition=available", "--timeout=10m", fmt.Sprintf("deploy/%s", "static-server-tcp"))
