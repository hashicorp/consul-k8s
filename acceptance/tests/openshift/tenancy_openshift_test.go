...
func TestOpenshift_APIGateway_Tenancy(t *testing.T) {
	...
	for _, c := range cases {
		...
		t.Run(name, func(t *testing.T) {
			...
			newOpenshiftCluster(t, cfg, c.secure, c.namespaceMirroring)
			...
			createReferenceGrant(t, cfg, k8sClient, "gateway-certificate", gatewayNamespace, certificateNamespace)
			createReferenceGrant(t, cfg, k8sClient, "route-service", routeNamespace, serviceNamespace)
...
func createReferenceGrant(t *testing.T, cfg *config.TestConfig, client client.Client, name, from, to string) {
	t.Helper()
	...
	helpers.Cleanup(t, cfg.NoCleanupOnFailure, cfg.NoCleanup, func() {
		cmd := exec.Command("kubectl", "delete", "referencegrant", name, "-n", to)
...
func applyFixture(t *testing.T, cfg *config.TestConfig, k8sOptions *terratestk8s.KubectlOptions, fixture string) {
	...
	helpers.Cleanup(t, cfg.NoCleanupOnFailure, cfg.NoCleanup, func() {
		fmt.Println(k8s.RunKubectlAndGetOutputE(t, k8sOptions, "delete", "-k", path.Join("../fixtures", fixture)))
	})
}
