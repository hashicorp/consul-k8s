...
func TestConnectInject_ExternalServers(t *testing.T) {
	for _, secure := range []bool{
		...
		t.Run(caseName, func(t *testing.T) {
			...
			if cfg.EnableWindows {
				k8s.RunKubectl(t, ctx.KubectlOptions(t), "exec", "deploy/"+connhelper.StaticServerName, "--", "cmd", "/c", "mkdir", "tmp", "&&", "cd", "tmp", "&&", "echo.", ">", "unhealthy")
			} else {
			...
			if cfg.EnableTransparentProxy {
				k8s.CheckStaticServerConnectionMultipleFailureMessages(t, ctx.KubectlOptions(t), connhelper.StaticClientName, false, []string{"curl: (56) Recv failure: Connection reset by peer", "curl: (56) Recv failure: Connection reset", "curl: (56) Recv failure: Connection was aborted", "curl: (52) Empty reply from server", "curl: (7) Failed to connect to static-server port 80: Connection refused"}, "", "http://static-server")
			} else {
