	...
	for _, secure := range []bool{
		...
		t.Run(caseName, func(t *testing.T) {
			...
			serverHelmValues := map[string]string{
				...
				"server.exposeService.nodePort.http":  "32500",
				"server.exposeService.nodePort.https": "32501",
				"server.exposeService.nodePort.grpc":  "32502",
			...
			dp2HelmValues := map[string]string{
				...
				"externalServers.httpsPort": "32500",
				"externalServers.grpcPort":  "32502",
			}
			...
			k8s.DeployKustomize(t, ctx2.KubectlOptions(t), cfg.NoCleanupOnFailure, cfg.NoCleanup, cfg.DebugDirectory, "../fixtures/cases/static-server-inject")
			if cfg.EnableTransparentProxy {
				k8s.DeployKustomize(t, ctx2.KubectlOptions(t), cfg.NoCleanupOnFailure, cfg.NoCleanup, cfg.DebugDirectory, "../fixtures/cases/static-client-tproxy")
			} else {
				k8s.DeployKustomize(t, ctx2.KubectlOptions(t), cfg.NoCleanupOnFailure, cfg.NoCleanup, cfg.DebugDirectory, "../fixtures/cases/static-client-inject")
			}

			// Check that both static-server and static-client have been injected in both clusters.
			for _, ctx := range []environment.TestContext{ctx1, ctx2} {
			...
			if cfg.EnableTransparentProxy {
				k8s.CheckStaticServerConnectionSuccessful(t, ctx2.KubectlOptions(t), connhelper.StaticClientName, "http://static-server")
			} else {
				k8s.CheckStaticServerConnectionSuccessful(t, ctx2.KubectlOptions(t), connhelper.StaticClientName, "http://localhost:1234")
			}
		})
	}
}
