	...
	for _, secure := range []bool{false, true} {
		...
		t.Run(name, func(t *testing.T) {
			...
			if cfg.EnableWindows {
				k8s.DeployKustomize(t, ctx.KubectlOptions(t), cfg.NoCleanupOnFailure, cfg.DebugDirectory, "../fixtures/cases/static-client-inject-windows")
			} else {
...
func TestConnectInject_MultiportServices(t *testing.T) {
	for _, secure := range []bool{false, true} {
		...
		t.Run(name, func(t *testing.T) {
			...
			if cfg.EnableWindows {
				k8s.DeployKustomize(t, ctx.KubectlOptions(t), cfg.NoCleanupOnFailure, cfg.DebugDirectory, "../fixtures/cases/static-server-inject-windows")
			} else {
			...
			if cfg.EnableWindows {
				...
				k8s.RunKubectl(t, ctx.KubectlOptions(t), "exec", "deploy/"+multiport, "--", "cmd", "/c", "mkdir", "tmp", "&&", "cd", "tmp", "&&", "echo.", ">", "unhealthy")
