...
func TestFailover_Connect_DefaultFailover(t *testing.T) {
	runFailoverConnect(t, false)
}

func TestFailover_Connect_SecureFailover(t *testing.T) {
	runFailoverConnect(t, true)
}

func runFailoverConnect(t *testing.T, ACLsEnabled bool) {
	t.Helper()
	...
	*/
	...
	for k, v := range testClusters {
		...
		if v.hasServer {
			testClusters[k].client, _ = testClusters[k].helmCluster.SetupConsulClient(t, ACLsEnabled)
		}
	}
