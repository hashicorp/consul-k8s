...
func TestWANFederation_Gateway(t *testing.T) {
	...
	t.Run("from primary to secondary", func(t *testing.T) {
		...
		if cfg.EnableWindows {
			k8s.DeployKustomize(t, secondaryContext.KubectlOptions(t), cfg.NoCleanupOnFailure, cfg.DebugDirectory, "../fixtures/cases/static-server-inject-windows")
		} else {
	...
	t.Run("from secondary to primary", func(t *testing.T) {
		...
		if cfg.EnableWindows {
			k8s.DeployKustomize(t, primaryContext.KubectlOptions(t), cfg.NoCleanupOnFailure, cfg.DebugDirectory, "../fixtures/cases/static-server-inject-windows")
		} else {
