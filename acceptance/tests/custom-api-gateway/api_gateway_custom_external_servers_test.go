...
func TestAPIGateway_ExternalServers(t *testing.T) {
	...
	helpers.WaitForHTTPRouteWithRetry(t, ctx.KubectlOptions(t), customGatewayHTTPRouteName, customGatewayFixturesDir, consulHTTPRouteResource) // use the full group name to avoid issues with multiple versions of gateway api

	logger.Log(t, "patching route to target server")
	k8s.RunKubectl(t, ctx.KubectlOptions(t), "patch", consulHTTPRouteResource, customGatewayHTTPRouteName, "-p", `{"spec":{"rules":[{"backendRefs":[{"name":"static-server","port":80}]}]}}`, "--type=merge")
