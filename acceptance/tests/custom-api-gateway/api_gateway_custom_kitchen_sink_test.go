...
func TestAPIGateway_KitchenSink(t *testing.T) {
	...
	fixturePath := "../fixtures/cases/custom-api-gateway/kitchen-sink"
	...
	logger.Log(t, "applying custom api-gateway resources")
	...
	gatewayAddress = waitForGatewayReady(t, ctx, k8sClient, customGatewayName, namespace, fixturePath, applyCounter)

	// Wait for HTTPRoute to be ready
	waitForHTTPRouteReady(t, ctx, k8sClient, customGatewayHTTPRouteName, namespace, fixturePath, applyCounter)

	if cfg.EnableOpenshift {
		logger.Log(t, "waiting for gateway and httproutes.consul.hashicorp.com to be ready")

		// Waiting for gateway to be ready.
		gatewayCounter := &retry.Counter{Count: 30, Wait: 10 * time.Second}
		logger.Log(t, "waiting for custom gateway to be ready")
		...
		logger.Log(t, "waiting for httproutes.consul.hashicorp.com to be ready")
		retry.RunWith(httpRouteCounter, t, func(r *retry.R) {
			err = k8sClient.Get(context.Background(), types.NamespacedName{Name: customGatewayHTTPRouteName, Namespace: namespace}, &httpRoute)
			require.NoError(r, err)
...
func waitForGatewayReady(t *testing.T, ctx environment.TestContext, k8sClient client.Client, gatewayName, namespace, fixturePath string, applyCounter *retry.Counter) string {
	...
	for attempt := range maxRetries {
		if attempt > 0 {
			...
			k8s.RunKubectl(t, ctx.KubectlOptions(t), "delete", consulGatewayResource, gatewayName, "--ignore-not-found=true")
...
func checkHTTPRouteReady(t *testing.T, k8sClient client.Client, routeName, namespace string) bool {
	...
	for i := 0; i < httpRouteCounter.Count; i++ {
		...
		if string(httpRoute.Status.Parents[0].ParentRef.Name) != customGatewayName {
...
func waitForHTTPRouteReady(t *testing.T, ctx environment.TestContext, k8sClient client.Client, routeName, namespace, fixturePath string, applyCounter *retry.Counter) {
	...
	for attempt := range maxRetries {
		if attempt > 0 {
			...
			k8s.RunKubectl(t, ctx.KubectlOptions(t), "delete", consulHTTPRouteResource, routeName, "--ignore-not-found=true")
