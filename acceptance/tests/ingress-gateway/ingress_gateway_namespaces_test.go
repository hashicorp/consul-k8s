	...
	for _, c := range cases {
		...
		t.Run(name, func(t *testing.T) {
			...
			if cfg.EnableWindows {
				k8s.DeployKustomize(t, nsK8SOptions, cfg.NoCleanupOnFailure, cfg.DebugDirectory, "../fixtures/cases/static-server-inject-windows")
			} else {
			...
			if cfg.EnableWindows {
				k8s.DeployKustomize(t, nsK8SOptions, cfg.NoCleanupOnFailure, cfg.DebugDirectory, "../fixtures/bases/static-client-windows")
			} else {
