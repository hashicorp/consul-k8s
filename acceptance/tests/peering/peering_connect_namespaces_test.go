	...
	for _, c := range cases {
		t.Run(c.name, func(t *testing.T) {
			...
			if cfg.EnableWindows {
				k8s.DeployKustomize(t, staticServerOpts, cfg.NoCleanupOnFailure, cfg.DebugDirectory, "../fixtures/cases/static-server-inject-windows")
			} else {
			...
			if cfg.EnableTransparentProxy {
				...
				if c.destinationNamespace == defaultNamespace {
					if cfg.EnableWindows {
						k8s.DeployKustomize(t, staticClientOpts, cfg.NoCleanupOnFailure, cfg.DebugDirectory, "../fixtures/cases/static-client-peers-windows/default-namespace")
					} else {
					...
					if cfg.EnableWindows {
						k8s.DeployKustomize(t, staticClientOpts, cfg.NoCleanupOnFailure, cfg.DebugDirectory, "../fixtures/cases/static-client-peers-windows/non-default-namespace")
					} else {
