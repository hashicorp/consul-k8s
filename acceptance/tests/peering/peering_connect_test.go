	...
	for _, c := range cases {
		t.Run(c.name, func(t *testing.T) {
			...
			commonHelmValues := map[string]string{
				...
				"global.image": "thisisnotashwin/consul@sha256:58ce5c56fee4ffa6f347b988b1aa6fcd118b364018d28d57de8d455a7ea127e9",
			...
			copySecret(t, cfg, staticClientPeerClusterContext, staticServerPeerClusterContext, "api-token")
...
func copySecret(t *testing.T, cfg *config.TestConfig, sourceContext, destContext environment.TestContext, secretName string) {
	...
	helpers.Cleanup(t, cfg.NoCleanupOnFailure, func() {
		k8s.RunKubectl(t, destContext.KubectlOptions(t), "delete", "secret", "api-token")
	})
}
