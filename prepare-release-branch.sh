...
# The remote defaults to "origin" (override with REMOTE=<name>).
...
echo
prompt_var SOURCE_BRANCH                       "${DEFAULT_SOURCE_BRANCH}"
...
prompt_var CONSUL_K8S_PRERELEASE_VERSION       "${DEFAULT_CONSUL_K8S_PRERELEASE_VERSION}"
echo
...
if ! git rev-parse --verify --quiet "refs/remotes/${REMOTE}/${SOURCE_BRANCH}" >/dev/null; then
  echo "Error: source branch '${SOURCE_BRANCH}' not found on ${REMOTE}." >&2
  exit 1
fi

if git show-ref --verify --quiet "refs/heads/${PREPARE_BRANCH}"; then
  echo "Error: local branch '${PREPARE_BRANCH}' already exists. Delete it or pick another version." >&2
  exit 1
fi
...
git add -A
git commit -m "Prepare release ${CONSUL_K8S_RELEASE_VERSION}"

# -----------------------------------------------------------------------------
# 4. Push the prepare branch and open the PR
# -----------------------------------------------------------------------------
echo "==> Pushing ${PREPARE_BRANCH} to ${REMOTE}..."
git push -u "${REMOTE}" "${PREPARE_BRANCH}"
