...
{{- define "consul.extraconfig" -}}
              cp /consul/extra-values/extra-from-values.json /consul/extra-config/extra-from-values-modified-env.json
              [ -n "${HOST_IP}" ] && sed -Ei "s|HOST_IP|${HOST_IP?}|g" /consul/extra-config/extra-from-values-modified-env.json
              [ -n "${POD_IP}" ] && sed -Ei "s|POD_IP|${POD_IP?}|g" /consul/extra-config/extra-from-values-modified-env.json
              [ -n "${HOSTNAME}" ] && sed -Ei "s|HOSTNAME|${HOSTNAME?}|g" /consul/extra-config/extra-from-values-modified-env.json
{{- end -}}
