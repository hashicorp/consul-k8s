...
@test "serverTLSInitCleanup/Job: tolerations not set by default" {
  cd `chart_dir`
  local actual=$(helm template \
      -s templates/tls-init-cleanup-job.yaml  \
      --set 'global.tls.enabled=true' \
...
@test "serverTLSInitCleanup/Job: tolerations can be set" {
  cd `chart_dir`
  local actual=$(helm template \
      -s templates/tls-init-cleanup-job.yaml  \
      --set 'global.tls.enabled=true' \
...
@test "serverTLSInitCleanup/Job: nodeSelector not set by default" {
  cd `chart_dir`
  local actual=$(helm template \
      -s templates/tls-init-cleanup-job.yaml  \
      --set 'global.tls.enabled=true' \
...
@test "serverTLSInitCleanup/Job: nodeSelector can be set" {
  cd `chart_dir`
  local actual=$(helm template \
      -s templates/tls-init-cleanup-job.yaml  \
      --set 'global.tls.enabled=true' \
