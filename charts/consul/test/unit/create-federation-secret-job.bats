...
@test "createFederationSecret/Job: -tls-server-name is not specified on command of get-auto-encrypt-client-ca init container when tls and auto-encrypt is enabled but global.cloud.enabled is not set" {
  cd `chart_dir`
...
@test "createFederationSecret/Job: -tls-server-name is specified on command of get-auto-encrypt-client-ca init container when tls and auto-encrypt is enabled and global.cloud.enabled is set" {
  cd `chart_dir`
  local object=$(helm template \
      -s templates/create-federation-secret-job.yaml  \
      --set 'global.federation.enabled=true' \
...
@test "createFederationSecret/Job: fails when global.cloud.enabled is set and global.cloud.secretName is not set" {
  cd `chart_dir`
  run helm template \
      -s templates/create-federation-secret-job.yaml  \
      --set 'global.federation.enabled=true' \
