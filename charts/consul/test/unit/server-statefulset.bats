...
@test "server/StatefulSet: creates HCP_RESOURCE_ID, HCP_CLIENT_ID, HCP_CLIENT_SECRET envvars in consul container when global.cloud.enabled is set" {
  cd `chart_dir`
  local object=$(helm template \
    -s templates/server-statefulset.yaml \
    --set 'global.cloud.enabled=true' \
...
@test "server/StatefulSet: creates HCP_AUTH_URL, HCP_SCADA_ADDRESS, and HCP_API_HOSTNAME envvars in consul container when global.cloud.enabled is set and those cloud values are specified" {
  cd `chart_dir`
  local object=$(helm template \
    -s templates/server-statefulset.yaml \
    --set 'global.cloud.enabled=true' \
...
@test "server/StatefulSet: fails when global.cloud.enabled is set and global.cloud.clientId.secretName is not set but global.cloud.clientSecret.secretName and global.cloud.resourceId.secretName is set" {
  cd `chart_dir`
  run helm template \
      -s templates/server-statefulset.yaml \
      --set 'global.cloud.enabled=true' \
...
@test "server/StatefulSet: fails when global.cloud.enabled is set and global.cloud.clientSecret.secretName is not set but global.cloud.clientId.secretName and global.cloud.resourceId.secretName is set" {
  cd `chart_dir`
  run helm template \
      -s templates/server-statefulset.yaml \
...
@test "server/StatefulSet: fails when global.cloud.enabled is set and global.cloud.resourceId.secretName is not set but global.cloud.clientId.secretName and global.cloud.clientSecret.secretName is set" {
  cd `chart_dir`
  run helm template \
      -s templates/server-statefulset.yaml  \
      --set 'connectInject.enabled=true' \
