...
@test "syncCatalog/Deployment: vault namespace annotations are set when tls is enabled and vaultNamespace is set" {
  cd `chart_dir`
  local cmd=$(helm template \
      -s templates/sync-catalog-deployment.yaml  \
...
@test "syncCatalog/Deployment: correct vault namespace annotations are set when tls is enabled and vaultNamespace is set and agentAnnotations are also set" {
  cd `chart_dir`
  local cmd=$(helm template \
      -s templates/sync-catalog-deployment.yaml  \
      --set 'apiGateway.enabled=true' \
...
@test "syncCatalog/Deployment: correct vault namespace is set when tls is enabled and vaultNamespace is set and additionalConfig is also set" {
  cd `chart_dir`
  local cmd=$(helm template \
      -s templates/sync-catalog-deployment.yaml  \
      --set 'apiGateway.enabled=true' \
