...
@test "serverACLInit/Job: podSecurityStandards default off" {
  cd `chart_dir`
  local actual=$(helm template \
      -s templates/server-acl-init-job.yaml  \
      --set 'global.acls.manageSystemACLs=true' \
      . | tee /dev/stderr |
      yq -r '.spec.template.spec.containers | map(select(.name == "post-install-job")) | .[0].securityContext | length > 0' | tee /dev/stderr)
  [ "${actual}" = "false" ]
}

@test "serverACLInit/Job: global.podSecurityStandards are not set when global.openshift.enabled=true" {
  cd `chart_dir`
  ...
  local actual=$(echo "$manifest" | yq -r '.spec.template.spec.containers | map(select(.name == "post-install-job")) | .[0].securityContext')
  [ "${actual}" = "null" ]
}

@test "serverACLInit/Job: global.podSecurityStandards can be set" {
  cd `chart_dir`
  local object=$(helm template \
      -s templates/server-acl-init-job.yaml  \
      --set 'global.acls.manageSystemACLs=true' \
  ...
  local actual=$(echo $object |
      yq -r '.containers | map(select(.name=="post-install-job")) | .[0].securityContext' | jq -r .bob)
  [ "${actual}" = "false" ]
  local actual=$(echo $object |
      yq -r '.containers | map(select(.name=="post-install-job")) | .[0].securityContext' | jq -r .alice)
  [ "${actual}" = "true" ]
}
