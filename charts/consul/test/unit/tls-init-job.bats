...
@test "tlsInit/Job: podSecurityStandards default off" {
  cd `chart_dir`
  local actual=$(helm template \
      -s templates/tls-init-job.yaml  \
      --set 'global.tls.enabled=true' \
      --set 'server.enabled=true' \
      . | tee /dev/stderr |
      yq -r '.spec.template.spec.containers | map(select(.name == "tls-init")) | .[0].securityContext | length > 0' | tee /dev/stderr)
  [ "${actual}" = "false" ]
}

@test "tlsInit/Job: global.podSecurityStandards are not set when global.openshift.enabled=true" {
  cd `chart_dir`
  ...
  local actual=$(echo "$manifest" | yq -r '.spec.template.spec.containers | map(select(.name == "tls-init")) | .[0].securityContext')
  [ "${actual}" = "null" ]
}

@test "tlsInit/Job: global.podSecurityStandards can be set with tls and acls enabled" {
  cd `chart_dir`
  ...
  local actual=$(echo $object |
      yq -r '.containers | map(select(.name=="tls-init")) | .[0].securityContext' | jq -r .bob)
  [ "${actual}" = "false" ]
  local actual=$(echo $object |
      yq -r '.containers | map(select(.name=="tls-init")) | .[0].securityContext' | jq -r .alice)
  [ "${actual}" = "true" ]
}
