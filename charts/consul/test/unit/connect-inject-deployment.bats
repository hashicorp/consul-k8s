...
@test "connectInject/Deployment: connect-inject has federation flag when federation is enabled" {
  cd `chart_dir`
  local object=$(helm template \
      -s templates/connect-inject-deployment.yaml \
      --set 'connectInject.enabled=true' \
      ...
      --set 'global.acls.manageSystemACLs=true' \
      . | tee /dev/stderr |
      yq '.spec.template.spec.containers[] | select(.name == "sidecar-injector")' | tee /dev/stderr)

  local actual=$(echo $object |
      yq -r '.command | any(contains("-enable-federation"))' | tee /dev/stderr)
  [ "${actual}" = "true" ]
}
