...
func LoadChart(chartFS embed.FS, chartDirName string) (chart.Charter, error) {
	...
	if err := extractChart(chartFS, chartDirName, tmpDir); err != nil {
		return nil, err
	}

	return loader.LoadDir(tmpDir)
}

func extractChart(chartFS embed.FS, chartDirName, targetDir string) error {
	return fs.WalkDir(chartFS, chartDirName, func(path string, entry fs.DirEntry, err error) error {
		if err != nil {
			return err
		}

		relPath := strings.TrimPrefix(path, chartDirName+"/")
...
func FetchChartValues(actionRunner HelmActionsRunner, namespace, name string, settings *helmCLI.EnvSettings, uiLogger func(string, ...interface{})) (map[string]interface{}, error) {
