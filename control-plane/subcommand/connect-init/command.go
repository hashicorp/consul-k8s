...
type Command struct {
	...
	// todo (agentless): this flag should eventually go away because we won't be writing ACL tokens to disk.
...
func (c *Command) Run(args []string) int {
	...
	// todo: return this from the consul pkg
	...
	if state.Token != "" {
		if err = common.WriteFileWithPerms(c.flagACLTokenSink, state.Token, 0644); err != nil {
			c.logger.Error("error writing token to file sink", "error", err)
			return 1
		}
	}
...
func (c *Command) validateFlags() error {
	...
	if c.consul.ConsulLogin.AuthMethod != "" && c.flagServiceAccountName == "" {
