...
func (c *Command) getGatewayRegistration(client *api.Client) backoff.Operation {
	...
	return func() error {
		...
		for _, gateway := range gatewayList.Services {
			switch gateway.Kind {
			case api.ServiceKindAPIGateway, api.ServiceKindMeshGateway, api.ServiceKindIngressGateway, api.ServiceKindTerminatingGateway, api.ServiceKindInferenceGateway:
				proxyID = gateway.ID
			}
		}
		if proxyID == "" {
