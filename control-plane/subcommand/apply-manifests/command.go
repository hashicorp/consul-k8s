...
func (c *Command) deleteGatewayAPICRDs() error {
	...
	l := hclog.New(&hclog.LoggerOptions{
		Name:   "consul-gateway-manifests",
		Level:  hclog.Debug, // or hclog.Info
		Output: os.Stdout,   // 👈 THIS IS KEY
	})
	for _, crd := range crds {
		...
		l.Info(fmt.Sprintf("✅ Deleted CRD: %s", crd))
...
const (
	synopsis = "Apply manifests (gateway.networking.k8s.io/v1) after installation/upgrade."
	help     = `
Usage: consul-k8s-control-plane apply-manifests [options]

  Creates the gateway.networking.k8s.io objects if exists already post-upgrade.
