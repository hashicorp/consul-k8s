...
// Package mcpserverresources implements the "mcp-server-resources" subcommand.
// It is invoked by a post-install/post-upgrade Helm Job and upserts the
// McpServerConfig CR from Helm values passed as CLI flags.
...
// Command is the "mcp-server-resources" subcommand.
type Command struct {
	UI cli.Ui
	...
	// Name of the McpServerConfig CR to create/update.
	flagConfigName string

	// Mirrors ai.mcpServer.enabled from values.yaml.
	flagEnabled bool

	// Mirrors ai.mcpServer.defaults.* from values.yaml.
...
func (c *Command) init() {
	...
	c.flags.StringVar(&c.flagConfigName, "config-name", "consul-mcp-server",
		"Name of the McpServerConfig CR to create or update.")
	...
	c.flags.StringVar(&c.flagComponent, "component", "mcp-server", "Helm chart component for created objects.")

	c.flags.BoolVar(&c.flagEnabled, "enabled", false,
		"Whether the MCP server interceptor feature is active (mirrors ai.mcpServer.enabled).")
	c.flags.IntVar(&c.flagInterceptorPort, "interceptor-port", 21101,
		"TCP port the MCP server container listens on inside the pod.")
	c.flags.StringVar(&c.flagTransport, "transport", "streamable-http",
		"MCP wire transport (streamable-http|sse|stdio).")
	c.flags.StringVar(&c.flagPath, "path", "/mcp",
		"HTTP path the MCP server listens on.")
	c.flags.StringVar(&c.flagProtocolVersion, "protocol-version", "2025-03-26",
		"MCP protocol version the server advertises.")
	c.flags.StringVar(&c.flagRequestsMemory, "requests-memory", "128Mi",
		"Memory request for the MCP server init-container.")
	c.flags.StringVar(&c.flagRequestsCPU, "requests-cpu", "250m",
		"CPU request for the MCP server init-container.")
	c.flags.StringVar(&c.flagLimitsMemory, "limits-memory", "256Mi",
		"Memory limit for the MCP server init-container.")
	c.flags.StringVar(&c.flagLimitsCPU, "limits-cpu", "500m",
		"CPU limit for the MCP server init-container.")
	...
	desired := &v1alpha1.McpServerConfig{
		...
		Spec: v1alpha1.McpServerConfigSpec{
			...
			Defaults: v1alpha1.McpServerDefaults{
				...
				Transport:       c.flagTransport,
				Path:            c.flagPath,
				ProtocolVersion: c.flagProtocolVersion,
				Resources:       resources,
			},
		},
	}

	if err := forceMcpServerConfig(c.ctx, c.k8sClient, desired); err != nil {
		c.UI.Error(fmt.Sprintf("Error upserting McpServerConfig %q: %s", c.flagConfigName, err))
		return 1
	}

	c.UI.Info(fmt.Sprintf("McpServerConfig %q successfully created/updated", c.flagConfigName))
	return 0
}

// forceMcpServerConfig upserts the McpServerConfig CR with exponential backoff.
func forceMcpServerConfig(ctx context.Context, k8sClient client.Client, desired *v1alpha1.McpServerConfig) error {
	return backoff.Retry(func() error {
		var existing v1alpha1.McpServerConfig
...
func (c *Command) validateFlags() error {
	...
	switch c.flagTransport {
	case "streamable-http", "sse", "stdio":
		// valid
	default:
		return fmt.Errorf("-transport must be one of streamable-http|sse|stdio, got %q", c.flagTransport)
	}
...
const (
	synopsis = "Create or update the McpServerConfig CR after Helm install/upgrade."
	help     = `
Usage: consul-k8s-control-plane mcp-server-resources [options]

  Upserts the McpServerConfig custom resource from the values supplied by
  the Helm chart. Intended to be run as a post-install/post-upgrade Helm Job.
