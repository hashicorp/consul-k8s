...
function build_consul_local {
   ...
   #
   # Note:
   #   The GOLDFLAGS, GOFIPS140, and GOTAGS environment variables will be used if set
   #   If the CONSUL_DEV environment var is truthy only the local platform/architecture is built.
   #   If the XC_OS or the XC_ARCH environment vars are present then only those platforms/architectures
   #   will be built. Otherwise all supported platform/architectures are built
   ...
   # FIPS 140-3 builds use the pure-Go in-tree Cryptographic Module (no cgo).
   # It is selected with GOFIPS140 and needs CGO_ENABLED=0.
   CGO_ENABLED=0

   echo "GOFIPS140: $GOFIPS140, GOTAGS: $GOTAGS CGO_ENABLED: $CGO_ENABLED" >> ~/debug.txt
   ...
   then
      ...
      CGO_ENABLED=${CGO_ENABLED} GOFIPS140=${GOFIPS140} gox \
         -os="${build_os}" \
         -arch="${build_arch}" \
         -ldflags="${GOLDFLAGS}" \
      ...
      do
         ...
         do
            ...
            CGO_ENABLED=${CGO_ENABLED} GOFIPS140=${GOFIPS140} GOOS=${os} GOARCH=${arch} go build -ldflags "${GOLDFLAGS}" -tags "${GOTAGS}" -o "${outdir}/${bin_name}"
            if test $? -ne 0
            then
