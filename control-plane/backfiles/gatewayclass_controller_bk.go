...
func (r *GatewayClassController) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	gc_exp := &gwv1beta1exp.GatewayClass{}
	err = r.Client.Get(ctx, req.NamespacedName, gc_exp)
	if err != nil {
		if k8serrors.IsNotFound(err) {
			return ctrl.Result{}, nil
		}
	...
	if string(gc_exp.Spec.ControllerName) != r.ControllerName {
		// This GatewayClass is not for this controller.
		_, err := RemoveFinalizer(ctx, r.Client, gc_exp, gatewayClassFinalizer)
		if err != nil {
			log.Error(err, "unable to remove finalizer")
		}
