...
func (c *Command) cmdAdd(args *skel.CmdArgs) error {
	...
	logger.Info("consul-cni plugin config", "config", cfg)
...
// TEST only
func findKubeconfigForTest(dir, base string) (string, error) {
	...
	if len(matches) == 0 {
		return "", fmt.Errorf("no kubeconfig files found matching %s", pattern)
	}

	// Pick the newest file by mod time (best guess)
...
func (c *Command) createK8sClient(cfg *PluginConf, logger hclog.Logger) error {
	...
	logger.Info("kubeconfig path - ", filepath.Join(cfg.CNINetDir, cfg.Kubeconfig))
	dir := cfg.CNINetDir
	base := cfg.Kubeconfig // "ZZZ-consul-cni-kubeconfig"

	logger.Info("resolving kubeconfig for test",
		"dir", dir,
		"base", base,
	)
	path, err := findKubeconfigForTest(dir, base)
	if err != nil {
		return err
	}

	logger.Info("using kubeconfig file (test)", "path", path)
