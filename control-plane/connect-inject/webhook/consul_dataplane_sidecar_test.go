...
func TestHandlerConsulDataplaneSidecar(t *testing.T) {
	...
	for name, c := range cases {
		t.Run(name, func(t *testing.T) {
			...
			require.Equal(t, container.Env[7].Name, "DP_CREDENTIAL_LOGIN_META")
			require.Equal(t, container.Env[7].Value, "pod-uid=$(POD_UID)")
			require.Equal(t, container.Env[8].Name, "DP_CREDENTIAL_LOGIN_META1")
			require.Equal(t, container.Env[8].Value, "pod=$(POD_NAMESPACE)/$(POD_NAME)")
			require.Equal(t, container.Env[9].Name, "DP_CREDENTIAL_LOGIN_META2")
			require.Equal(t, container.Env[9].Value, "pod-uid=$(POD_UID)")
		})
	}
}
