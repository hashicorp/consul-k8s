...
func mergeDeployments(log logr.Logger, gcc v1alpha1.GatewayClassConfig, gateway gwv1beta1.Gateway, a, b *appsv1.Deployment) *appsv1.Deployment {
	managedFieldsEqual := compareDeployments(a, b)
	if !managedFieldsEqual {
		log.Info(
			"managed gateway deployment fields changed; replacing pod template",
			"desiredTemplateHash", deploymentTemplateHash(a),
			"existingTemplateHash", deploymentTemplateHash(b),
			"desiredImage", primaryContainerImage(a),
			"existingImage", primaryContainerImage(b),
		)
		// Replace template
		b.Spec.Template = a.Spec.Template
		b.Spec.Replicas = deploymentReplicas(gcc, a.Spec.Replicas)
	} else if !cmp.Equal(a.Spec.Template, b.Spec.Template) {
		log.Info(
			"preserving existing pod template because managed fields are unchanged",
			"desiredTemplateHash", deploymentTemplateHash(a),
			"existingTemplateHash", deploymentTemplateHash(b),
			"desiredImage", primaryContainerImage(a),
			"existingImage", primaryContainerImage(b),
