...
func (v fakeReferenceValidator) GatewayCanReferenceSecret(gateway gwv1.Gateway, secretRef gwv1.SecretObjectReference) bool {
	return true
}

func (v fakeReferenceValidator) HTTPRouteCanReferenceBackend(httproute gwv1.HTTPRoute, backendRef gwv1.BackendRef) bool {
	return true
}
...
func TestTranslator_ToAPIGateway(t *testing.T) {
	...
	testCases := map[string]struct {
		...
		listenerOneK8sCertRefs []gwv1.SecretObjectReference
		listenerOneTLSOptions  map[gwv1.AnnotationKey]gwv1.AnnotationValue
	}{
		"gw name": {
			...
			listenerOneK8sCertRefs: []gwv1.SecretObjectReference{
				{
					Name:      gwv1.ObjectName(listenerOneCertName),
					Namespace: PointerTo(gwv1.Namespace(listenerOneCertK8sNamespace)),
				},
			},
			listenerOneTLSOptions: map[gwv1.AnnotationKey]gwv1.AnnotationValue{
				TLSMaxVersionAnnotationKey:   gwv1.AnnotationValue(listenerOneMaxVersion),
				TLSMinVersionAnnotationKey:   gwv1.AnnotationValue(listenerOneMinVersion),
				TLSCipherSuitesAnnotationKey: gwv1.AnnotationValue(strings.Join(listenerOneCipherSuites, ",")),
			},
		},
		"when k8s has certs that are not referenced in consul": {
			...
			listenerOneK8sCertRefs: []gwv1.SecretObjectReference{
				...
				{
					Name:      gwv1.ObjectName("cert that won't exist in the translated type"),
					Namespace: PointerTo(gwv1.Namespace(listenerOneCertK8sNamespace)),
				},
			},
			listenerOneTLSOptions: map[gwv1.AnnotationKey]gwv1.AnnotationValue{
					...
					Listeners: []gwv1.Listener{
						{
							...
							Hostname: PointerTo(gwv1.Hostname(listenerOneHostname)),
						...
						},
						{
							...
							Hostname: PointerTo(gwv1.Hostname(listenerTwoHostname)),
							Port:     gwv1.PortNumber(listenerTwoPort),
							Protocol: gwv1.ProtocolType(listenerTwoProtocol),
							TLS: &gwv1.ListenerTLSConfig{
								CertificateRefs: []gwv1.SecretObjectReference{
									{
										Name:      gwv1.ObjectName(listenerTwoCertName),
										Namespace: PointerTo(gwv1.Namespace(listenerTwoCertK8sNamespace)),
									},
								},
							},
						},
					...
					Spec: gwv1.HTTPRouteSpec{
						CommonRouteSpec: gwv1.CommonRouteSpec{
							ParentRefs: []gwv1.ParentReference{
								{
									Namespace:   PointerTo(gwv1.Namespace("k8s-gw-ns")),
									Name:        gwv1.ObjectName("api-gw"),
									Kind:        PointerTo(gwv1.Kind("Gateway")),
									SectionName: PointerTo(gwv1.SectionName("listener-1")),
								},
							},
						},
								...
								BackendRefs: []gwv1.HTTPBackendRef{
									{
										BackendRef: gwv1.BackendRef{
											BackendObjectReference: gwv1.BackendObjectReference{
												Name:      "service one",
												Namespace: PointerTo(gwv1.Namespace("other")),
											},
								...
								BackendRefs: []gwv1.HTTPBackendRef{
									{
										BackendRef: gwv1.BackendRef{
											BackendObjectReference: gwv1.BackendObjectReference{
												Name:      "service one",
												Namespace: PointerTo(gwv1.Namespace("some ns")),
											},
											Weight: PointerTo(int32(45)),
										},
										Filters: []gwv1.HTTPRouteFilter{
					...
					Spec: gwv1.HTTPRouteSpec{
						CommonRouteSpec: gwv1.CommonRouteSpec{
							ParentRefs: []gwv1.ParentReference{
								...
								{
									Namespace:   PointerTo(gwv1.Namespace("k8s-gw-ns")),
									Name:        gwv1.ObjectName("consul don't know about me"),
									Kind:        PointerTo(gwv1.Kind("Gateway")),
									SectionName: PointerTo(gwv1.SectionName("listener-1")),
								},
							},
...
// TestResolveListenerProtocol verifies that the annotation-based protocol
// override selects the correct Consul listener protocol (RFC-0002).
func TestResolveListenerProtocol(t *testing.T) {
	t.Parallel()

	makeGateway := func(annotations map[string]string) gwv1.Gateway {
		return gwv1.Gateway{
			ObjectMeta: metav1.ObjectMeta{
				Name:        "edge",
				Namespace:   "default",
	...
	makeListener := func(name string, protocol gwv1.ProtocolType) gwv1.Listener {
		return gwv1.Listener{Name: gwv1.SectionName(name), Protocol: protocol}
	}
	...
	tests := []struct {
	...
	}{
		{
			...
			listener:     makeListener("grpc-listener", gwv1.HTTPProtocolType),
			wantProtocol: "grpc",
		},
		{
			...
			listener:     makeListener("h2", gwv1.HTTPProtocolType),
			wantProtocol: "http2",
		},
		{
		...
		},
		{
			...
			listener:     makeListener("grpc-listener", gwv1.HTTPProtocolType),
			wantProtocol: "http", // falls back to listenerProtocolMap["http"]
		},
		{
			name:         "no annotation uses protocol map — HTTP",
			listener:     makeListener("web", gwv1.HTTPProtocolType),
			wantProtocol: "http",
		},
		{
			name:         "no annotation uses protocol map — HTTPS",
			listener:     makeListener("secure", gwv1.HTTPSProtocolType),
			wantProtocol: "http",
		},
		{
			name:         "no annotation uses protocol map — TCP",
			listener:     makeListener("tcp", gwv1.TCPProtocolType),
			wantProtocol: "tcp",
		},
		{
	...
	for _, tt := range tests {
		...
		t.Run(tt.name, func(t *testing.T) {
			t.Parallel()
			gw := makeGateway(tt.gatewayAnnotations)
			got := resolveListenerProtocol(gw, tt.listener)
			require.Equal(t, tt.wantProtocol, got)
		})
	}
}

// TestToAPIGatewayListener_ProtocolAnnotation verifies that the annotation-based
// protocol selection is propagated into the Consul api-gateway listener (RFC-0002).
func TestToAPIGatewayListener_ProtocolAnnotation(t *testing.T) {
	t.Parallel()
		...
		t.Run(tc.annotation, func(t *testing.T) {
			...
			gateway := gwv1.Gateway{
				ObjectMeta: metav1.ObjectMeta{
					...
					Namespace: "default",
					Annotations: map[string]string{
						ListenerProtocolAnnotationPrefix + "listener" + ListenerProtocolAnnotationSuffix: tc.annotation,
					},
				},
			}
			listener := gwv1.Listener{
			...
			require.Equal(t, tc.wantProtocol, got.Protocol,
				"expected Consul listener protocol %q from annotation %q", tc.wantProtocol, tc.annotation)
		})
	}
}
