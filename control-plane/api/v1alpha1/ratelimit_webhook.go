...
//
// +kubebuilder:webhook:verbs=create;update,path=/mutate-v1alpha1-ratelimits,mutating=true,failurePolicy=fail,groups=consul.hashicorp.com,resources=ratelimits,versions=v1alpha1,name=mutate-ratelimits.consul.hashicorp.com,sideEffects=None,admissionReviewVersions=v1beta1;v1

func (v *RateLimitWebhook) Handle(ctx context.Context, req admission.Request) admission.Response {
	var limit RateLimit
	var limitList RateLimitList
	err := v.decoder.Decode(req, &limit)
	if err != nil {
		return admission.Errored(http.StatusBadRequest, err)
	}

	if v.EnablePartitions && !v.HasGlobalConfigACLToken {
	...
	if req.Operation == admissionv1.Create {
		...
		if limit.KubernetesName() != "global" {
...
func (v *RateLimitWebhook) List(ctx context.Context) ([]common.ConfigEntryResource, error) {
	var limitList RateLimitList
	if err := v.Client.List(ctx, &limitList); err != nil {
		return nil, err
	}
...
func (v *RateLimitWebhook) SetupWithManager(mgr ctrl.Manager) {
	v.decoder = admission.NewDecoder(mgr.GetScheme())
	mgr.GetWebhookServer().Register("/mutate-v1alpha1-ratelimits", &admission.Webhook{Handler: v})
}
