...
func TestExportedServices_Validate(t *testing.T) {
	cases := map[string]struct {
		...
		"no consumers specified": {
			...
			partitionsEnabled: true,
			expectedErrMsgs: []string{
				`spec.services[0]: Invalid value: []v1.ServiceConsumer{}: service must have at least 1 consumer.`,
			},
		},
		"both partition and peer name specified": {
			...
			partitionsEnabled: true,
			expectedErrMsgs: []string{
				`spec.services[0].consumers[0]: Invalid value: v1.ServiceConsumer{Partition:"second", Peer:"second-peer"}: both partition and peer cannot be specified.`,
			},
		},
		"neither partition nor peer name specified": {
			...
			partitionsEnabled: true,
			expectedErrMsgs: []string{
				`spec.services[0].consumers[0]: Invalid value: v1.ServiceConsumer{Partition:"", Peer:""}: either partition or peer must be specified.`,
			},
		},
		...
		"multiple errors": {
			...
			expectedErrMsgs: []string{
				`spec.services[0].consumers[0]: Invalid value: v1.ServiceConsumer{Partition:"second", Peer:"second-peer"}: both partition and peer cannot be specified.`,
				`spec.services[0].consumers[1]: Invalid value: v1.ServiceConsumer{Partition:"", Peer:""}: either partition or peer must be specified.`,
			},
		},
	}
