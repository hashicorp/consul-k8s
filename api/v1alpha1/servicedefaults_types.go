...
func (in *ServiceDefaults) Validate(namespacesEnabled bool) error {
	...
	// Explicitly set partitions enabled to false as we don't intend on it being used.
	allErrs = append(allErrs, in.Spec.UpstreamConfig.validate(path.Child("upstreamConfig"), false)...)
